...
fn test_removal_of_nonexistent_key() {
    ...
    // Allocate a handle but don't insert
    // 分配 handle 但不插入
    let h = map.allocate_handle();
    let k = h.key();

    // Attempt removal - should return None as slot is reserved but not occupied
    // 尝试删除 - 应该返回 None，因为 slot 是 reserved 但未 occupied
