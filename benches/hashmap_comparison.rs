...
fn bench_deferred_insertion(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("DeferredMap_deferred", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut map = DeferredMap::<usize>::new();
        ...
        group.bench_with_input(
            BenchmarkId::new("DeferredMap_direct", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut map = DeferredMap::<usize>::new();
        ...
        group.bench_with_input(
            BenchmarkId::new("FxHashMap_deferred", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut map = FxHashMap::default();
        ...
        // FxHashMap: 只能直接插入
        group.bench_with_input(
            BenchmarkId::new("FxHashMap_direct", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut map = FxHashMap::default();
...
fn bench_clone(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("DeferredMap", size), size, |b, _| {
            b.iter(|| black_box(deferred_map.clone()));
        });
